<script lang="ts">
	import type { ResourceKey } from '@aegis/core';
	...
	function toggleStrategyNotes() {
		strategyNotesOpen = !strategyNotesOpen;
	}

	function saveToLocal() {
		if (!buildOrder) return;
		customBuildsStore.save(buildOrder);
		saved = true;
	}

	onMount(() => {
		customBuildsStore.load();
	});
</script>
...
{#if !buildOrder}
	<!-- ── Error state ────────────────────────────────────────────────────────── -->
	<main
		class="min-h-screen bg-stone-950 text-stone-100 flex flex-col items-center justify-center px-4 py-10 gap-6"
	>
		<div
			class="w-full max-w-lg bg-red-950 border-2 border-red-500 rounded-2xl p-6 flex flex-col gap-4"
			role="alert"
		>
...
{:else}
	...
	<main class="min-h-screen bg-stone-950 text-stone-100 flex flex-col items-center px-4 py-8 gap-6">
		...
		<header class="w-full max-w-lg flex items-start gap-4">
			...
			<div class="flex-1 min-w-0">
				<p class="text-xs text-stone-500 uppercase tracking-widest font-semibold">
					{game?.icon ?? ''} {game?.name ?? buildOrder.gameId}
		...
		{#if buildOrder.strategy_notes && buildOrder.strategy_notes.length > 0}
			<div class="w-full max-w-lg">
				<button
				...
				{#if strategyNotesOpen}
					<div
						class="mt-2 rounded-xl bg-stone-900 border border-stone-700 divide-y divide-stone-800 overflow-hidden"
					>
		...
		>
			...
			<div class="flex items-center gap-3">
				<span
					class="bg-amber-500 text-stone-950 text-sm font-bold w-9 h-9 rounded-full flex items-center justify-center shrink-0 shadow-lg shadow-amber-500/30"
				>
					{currentStep.id}
				</span>
			...
			<div class="grid grid-cols-3 sm:grid-cols-5 gap-2">
				{#each resourceKeys as key}
					...
					{@const value =
						key === 'villagerCount' ? currentStep.villagerCount : currentStep[key as ResourceKey]}
					{#if value !== undefined}
